...
class ACYSMSacltableType{
	...
	function display($category,$actions){
		...
		foreach($actions as $action){
			$trans = JText::_('SMS_'.strtoupper($action));
		...
		foreach($this->groups as $oneGroup){
			...
			foreach($actions as $action){
				...
				if(ACYSMS_J16 || !in_array($oneGroup->value,array(29,30))){
					if(ACYSMS::isAllowed($value,$oneGroup->value)){
		...
		$openDiv = JHTML::_('acysmsselect.radiolist', $this->choice, "acl_$category", 'onclick="updateACLTable(\''.$category.'\',\''.$oneAction.'\');"', 'value', 'text',($custom ? 'special' : 'all'));
