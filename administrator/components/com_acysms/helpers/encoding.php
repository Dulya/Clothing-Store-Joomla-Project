...
class ACYSMSencodingHelper{
	...
	function change($data,$input,$output){
		...
		if(function_exists('iconv')){
			set_error_handler('acysms_error_handler_encoding');
			$encodedData = iconv($input, $output."//IGNORE", $data);
			restore_error_handler();
			if(!empty($encodedData) && !acysms_error_handler_encoding('result')){
				return $encodedData;
			}
		}
...
function acysms_error_handler_encoding($errno,$errstr=''){
	static $error = false;
