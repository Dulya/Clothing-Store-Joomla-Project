...
if(!ACYSMS_J16){
	...
	class JElementCustomfields extends JElement
	{
		function fetchElement($name, $value, &$node, $control_name)
		{
			...
			$text .= '<a class="modal" id="link'.$control_name.'customfields" title="'.JText::_('SMS_EXTRA_FIELDS').'"  href="'.$link.'" rel="{handler: \'iframe\', size: {x: 650, y: 375}}"><button class="acysms_button" onclick="return false">'.JText::_('Select').'</button></a>';
	...
	class JFormFieldCustomfields extends JFormField
	{
		...
		function getInput() {
			...
			$text .= '<a class="modal" id="link'.$this->name.'customfields" title="'.JText::_('SMS_EXTRA_FIELDS').'"  href="'.$link.'" rel="{handler: \'iframe\', size: {x: 650, y: 375}}"><button class="acysms_button" onclick="return false">'.JText::_('Select').'</button></a>';
